...
exports.CosmeticConstructor = void 0;
class CosmeticConstructor {
    /**
    ...
    summaryTableY(settings) {
        ...
        for (const [key, value] of Object.entries(settings.table)) {
            if (String(key).length > minlength)
                minlength = String(key).length;
            value.forEach((element) => {
                if (String(element).length > minlength)
                    minlength = String(element).length;
            });
        }
        ...
        if (settings.tableName != undefined)
            visualTable += '| ' + settings.tableName + '\n';
        ...
        var maxlines = 0;
        for (const [key] of Object.entries(settings.table)) {
            if (settings.table[key].length > maxlines)
                maxlines = settings.table[key].length;
        }
        for (let line = 0; line < maxlines; line++) {
            ...
            for (const [key] of Object.entries(settings.table)) {
                ...
                if (settings.table[key].length > maxlines)
                    maxlines = settings.table[key].length;
                let arr = [];
                settings.table[key].forEach((dir) => {
                    arr.push(dir);
                });
...
exports.CosmeticConstructor = CosmeticConstructor;
